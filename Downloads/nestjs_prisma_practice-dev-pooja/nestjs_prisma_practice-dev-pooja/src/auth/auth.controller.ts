...
export class AuthController {
  ...
  async Sort(@Req() req, @Res() res) {
    ...
    const data = await this.authService.sort(req.query, res);

    res.send(data);
  }

  @Post('/page')
  async Pagination(@Req() req, @Res() res, @Query('pagenum') pagenum: number) {
    // console.log('findAll');

    const data= await this.authService.pagination(req, res, pagenum);
    res.send(data);
  }
